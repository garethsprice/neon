 ...
 * @param {number} [options.stepNumber] - Step number to display (1-indexed)
 * @param {number} [options.value=0] - Initial value: 0 (off), 1 (normal), 2 (accented)
 * @param {boolean} [options.hasFlam=false] - Whether step has a flam
 * @param {boolean} [options.showFlam=false] - Whether to show flam editing UI
 * @param {boolean} [options.isActive=false] - Whether this is the current playhead position
 * @param {Function} [options.onClick] - Click handler, receives current value
 * @param {Function} [options.onFlamToggle] - Flam toggle handler (shift+click behavior)
 * @returns {Object} { element, getValue, setValue, setActive, setHasFlam, setShowFlam, triggerHit, destroy }
 */
export function createStepButton(options = {}) {
    injectStyles();
    ...
    function updateButtonClass() {
        ...
        if (showFlam) button.classList.add('show-flam');
 ...
 * @param {Object} options
 * @param {number} [options.steps=16] - Number of steps
 * @param {number[]} [options.values=[]] - Initial values for each step
 * @param {boolean[]} [options.flams=[]] - Initial flam states
 * @param {Function} [options.onStepClick] - Handler for step clicks (stepIndex, currentValue)
 * @param {Function} [options.onFlamToggle] - Handler for flam toggles (stepIndex)
 * @returns {Object} { element, getValues, setValues, setActiveStep, setShowFlams, triggerStep }
 */
export function createStepGrid(options = {}) {
    injectStyles();
    ...
    return {
        ...
        setValues: (vals) => {
            stepButtons.forEach((s, i) => s.setValue(vals[i] || 0));
        },
        setFlams: (flamArray) => {
        ...
        triggerStep: (index, duration = 100) => {
