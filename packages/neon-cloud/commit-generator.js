/**
 * neon-cloud/commit-generator.js
 ...
 * @param {string} [appType='generic'] - App type: 'drums', 'synth', 'noise', or 'generic'
 * @param {object} [options] - Configuration options
 * @param {number} [options.maxLength=60] - Maximum message length
 * @param {boolean} [options.useAI=true] - Whether to use AI for generation
 * @returns {Function} Async function that generates commit messages
 */
export function createCommitMessageGenerator(appType = 'generic', options = {}) {
    const { maxLength = 60, useAI = true } = options;
    const prompts = APP_PROMPTS[appType] || APP_PROMPTS.generic;

    /**
     * Generate a commit message based on changes
     * @param {object} changes - Change summary from CloudStore diff
     * @param {object} prevData - Previous state data
     * @param {object} currData - Current state data
     * @param {object} [commitOptions] - Additional options
     * @param {boolean} [commitOptions.isRemix] - Whether this is a remix
     * @param {object} [commitOptions.remixSource] - Source track info if remix
     * @returns {Promise<string>} Generated commit message
     */
    return async function generateCommitMessage(changes, prevData, currData, commitOptions = {}) {
        // Handle remix case
 ...
 * @param {boolean} [options.isRemix] - Whether this is a remix
 * @param {object} [options.remixSource] - Source track info if remix
 * @returns {string} Generated commit message
 */
export function generateSimpleCommitMessage(changes, prevData, currData, options = {}) {
    // Handle remix case
    ...
    if (changes.scalar) {
        for (const [key, change] of Object.entries(changes.scalar)) {
            if (typeof change.prev === 'number' && typeof change.curr === 'number') {
                const prevPct = Math.round(change.prev * 100);
                const currPct = Math.round(change.curr * 100);
