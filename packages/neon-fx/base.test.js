...
import { MockAudioContext, createMockAudioContext } from './__mocks__/web-audio.js';
...
describe('AudioPlugin', () => {
    ...
    describe('constructor', () => {
        it('throws error when AudioContext is not provided', () => {
            expect(() => new AudioPlugin()).toThrow('AudioPlugin requires an AudioContext');
            expect(() => new AudioPlugin(null)).toThrow('AudioPlugin requires an AudioContext');
        });

        it('creates input and output gain nodes', () => {
            const plugin = new AudioPlugin(audioContext);
    ...
    describe('connect/disconnect', () => {
        ...
        it('connects to another AudioPlugin', () => {
            const plugin1 = new AudioPlugin(audioContext);
            const plugin2 = new AudioPlugin(audioContext);
        ...
        it('disconnects from specific destination', () => {
            ...
            plugin.connect(dest1);
            plugin.connect(dest2);
            expect(plugin._output._connections.length).toBe(2);

            plugin.disconnect(dest1);
            expect(plugin._output._connections.length).toBe(1);
        ...
        it('disconnects all when no destination specified', () => {
            const plugin = new AudioPlugin(audioContext);
            plugin.connect(audioContext.createGain());
            plugin.connect(audioContext.createGain());
    ...
    describe('_setAudioParam helper', () => {
        it('sets value immediately when rampTime is 0', () => {
            ...
            plugin._setAudioParam(gainNode.gain, 0.5, 0);
        ...
        it('uses setTargetAtTime when rampTime > 0', () => {
            ...
            plugin._setAudioParam(gainNode.gain, 0.5, 0.3);
...
describe('setupBypassRouting', () => {
    ...
    it('connects dry path (bypass)', () => {
        ...
        setupBypassRouting(plugin, processingInput, processingOutput);
