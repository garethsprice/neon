...
/** Options for creating a feed item */
export interface FeedItemOptions {
  ...
  onLike?: ((liked: boolean) => void) | null;
  /** Remix button click handler */
  onRemix?: (() => void) | null;
 ...
 */
export function createFeedItem(options: FeedItemOptions = {}): FeedItemComponent {
  injectStyles();
  ...
  if (onPlay) {
    playStat.addEventListener('click', (e: MouseEvent) => {
      e.stopPropagation();
      onPlay();
    });
  }
  ...
  if (onLike) {
    likeStat.addEventListener('click', (e: MouseEvent) => {
      e.stopPropagation();
      onLike(!isLiked);
    });
  }
  ...
  if (onHistory) {
    ...
    historyBtn.addEventListener('click', (e: MouseEvent) => {
      e.stopPropagation();
      onHistory();
    });
  ...
  if (onRemix) {
    ...
    remixBtn.addEventListener('click', (e: MouseEvent) => {
      e.stopPropagation();
      onRemix();
    });
  ...
  return {
    ...
    setLikes: (count: number): void => {
      likeCount = count;
      const likeCountEl = likeStat.querySelector('.like-count');
...
/** Options for creating a feed container */
export interface FeedOptions {
  /** Array of feed item configs */
...
/** Feed container component interface */
...
export function createFeed(options: FeedOptions = {}): FeedComponent {
  ...
  let itemRefs: FeedItemComponent[] = [];
  ...
  const pageInfo = pagination.querySelector('.neon-feed-pagination-info') as HTMLElement;

  function renderItems(itemsArray: FeedItemOptions[]): void {
  ...
  return {
    element,
    container,
    setItems: (newItems: FeedItemOptions[]): void => {
    ...
    setPage: (newPage: number, newTotal: number = totalPages): void => {
      currentPage = newPage;
      totalPages = newTotal;
      updatePagination();
    },
